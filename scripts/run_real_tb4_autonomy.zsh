...
usage() {
  cat <<'EOF'
Usage:
  scripts/run_real_tb4_autonomy.zsh tb4_2 [options]
  scripts/run_real_tb4_autonomy.zsh tb4_5 [options]
...
Examples:
  scripts/run_real_tb4_autonomy.zsh tb4_2
  scripts/run_real_tb4_autonomy.zsh tb4_5 --dry-run=true --launch=false
EOF
}
...
bool_value() {
  case "$1" in
    true|True|TRUE|1|yes|YES|y) print true ;;
    false|False|FALSE|0|no|NO|n) print false ;;
...
wait_for_topic_list() {
  ...
  for idx in $(seq 1 "$tries"); do
    ros2 topic list > "$TOPIC_LIST_FILE" 2> "$TOPIC_ERROR_FILE" && [[ -s "$TOPIC_LIST_FILE" ]] && return 0
    sleep 1
  done
...
[[ -f /opt/ros/jazzy/setup.zsh ]] || fail "Missing /opt/ros/jazzy/setup.zsh"
source /opt/ros/jazzy/setup.zsh

[[ -f install/setup.zsh ]] || fail "Missing install/setup.zsh. Run: colcon build --symlink-install"
...
topic_exists "$odom_topic" || fail "Missing odom topic $odom_topic. Available topics saved in $TOPIC_LIST_FILE during this run."
...
if [[ "$reset_odom" == true ]]; then
  ...
  if ros2 service list | grep -qx -- "$reset_service" || timeout 8 zsh -lc "source /opt/ros/jazzy/setup.zsh && source '$repo_root/install/setup.zsh' && until ros2 service list | grep -qx -- '$reset_service'; do sleep 1; done"; then
