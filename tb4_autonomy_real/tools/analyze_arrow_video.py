...
from tb4_autonomy_real.data_types import FrameContext
...
def _params_from_yaml(path: Path) -> dict:
    ...
    return data.get('real_vision_controller_node', {}).get('ros__parameters', {})
...
def _arrow_config(params: dict) -> ArrowDetectorConfig:
    return ArrowDetectorConfig(
        ...
        use_paper_candidate=_as_bool(params.get('arrow_use_paper_candidate'), True),
        black_frame_enable=_as_bool(params.get('arrow_black_frame_enable'), True),
        black_frame_min_area_ratio=float(params.get('arrow_black_frame_min_area_ratio', 0.002)),
        black_frame_max_area_ratio=float(params.get('arrow_black_frame_max_area_ratio', 0.30)),
        black_frame_approx_epsilon_ratio=float(params.get('arrow_black_frame_approx_epsilon_ratio', 0.04)),
        black_frame_min_border_density=float(params.get('arrow_black_frame_min_border_density', 0.08)),
        black_frame_max_inner_density=float(params.get('arrow_black_frame_max_inner_density', 0.35)),
        black_frame_min_border_inner_contrast=float(params.get('arrow_black_frame_min_border_inner_contrast', 0.03)),
        black_frame_border_band_ratio=float(params.get('arrow_black_frame_border_band_ratio', 0.12)),
        black_frame_close_kernel=int(params.get('arrow_black_frame_close_kernel', 5)),
        black_frame_dilate_kernel=int(params.get('arrow_black_frame_dilate_kernel', 3)),
        black_frame_min_score=float(params.get('arrow_black_frame_min_score', 0.25)),
        black_frame_refine_with_white_paper=_as_bool(
            params.get('arrow_black_frame_refine_with_white_paper'), True
        ),
        black_frame_white_roi_padding_ratio=float(params.get('arrow_black_frame_white_roi_padding_ratio', 0.18)),
        black_frame_white_min_area_ratio=float(params.get('arrow_black_frame_white_min_area_ratio', 0.002)),
        black_frame_white_max_area_ratio=float(params.get('arrow_black_frame_white_max_area_ratio', 0.35)),
        black_frame_white_max_area_expand_ratio=float(
            params.get('arrow_black_frame_white_max_area_expand_ratio', 1.8)
        ),
        black_frame_white_min_overlap_ratio=float(params.get('arrow_black_frame_white_min_overlap_ratio', 0.25)),
        black_frame_white_center_tolerance_ratio=float(
            params.get('arrow_black_frame_white_center_tolerance_ratio', 0.35)
        ),
        black_frame_white_approx_epsilon_ratio=float(
            params.get('arrow_black_frame_white_approx_epsilon_ratio', 0.035)
        ),
        black_frame_white_open_kernel=int(params.get('arrow_black_frame_white_open_kernel', 3)),
        black_frame_white_close_kernel=int(params.get('arrow_black_frame_white_close_kernel', 5)),
        ...
        black_fragment_min_box_h_px=float(params.get('arrow_black_fragment_min_box_h_px', 0.0)),
        ...
        use_base_tip_direction=_as_bool(params.get('arrow_use_base_tip_direction'), False),
...
def _draw_arrow_analysis_overlay(image, detection, debug: dict) -> None:
    height, width = image.shape[:2]
    cv2.line(image, (width // 2, 0), (width // 2, height), (80, 80, 80), 1)
    ...
    cv2.rectangle(image, p1, p2, (0, 180, 255), 1)
    cv2.line(
        image,
        (width // 2, int(center[1])),
        (int(round(center[0])), int(round(center[1]))),
        (0, 255, 255),
        1,
    )
    cv2.circle(image, (int(round(center[0])), int(round(center[1]))), 4, (0, 255, 255), -1)

    if detection.heading_base is not None and detection.heading_tip is not None:
        base = tuple(int(value) for value in detection.heading_base)
        tip = tuple(int(value) for value in detection.heading_tip)
        cv2.arrowedLine(image, base, tip, (255, 255, 0), 2, tipLength=0.35)

    raw = 'none' if detection is None else detection.raw_direction
    stable = 'T' if detection.is_stable else 'F'
    bottom_ratio = (box.y + box.h) / float(max(1, height))
    label = (
        f'raw={raw} stable={stable} '
        f'area={detection.area_ratio:.3f} bottom={bottom_ratio:.2f} '
        f'frame={float(getattr(detection, "frame_score", 0.0)):.2f} '
        f'white={str(bool(getattr(detection, "refined_with_white_paper", False)))[0]}'
    )
    cv2.rectangle(image, (0, height - 18), (width, height), (0, 0, 0), -1)
    cv2.putText(
        image,
        label,
        (4, height - 5),
        cv2.FONT_HERSHEY_SIMPLEX,
        max(0.30, min(0.42, width / 700.0)),
        (255, 255, 255),
        1,
        cv2.LINE_AA,
    )

    target = f'cmd v={float(debug.get("linear_x", 0.0)):.3f} w={float(debug.get("angular_z", 0.0)):.3f}'
    cv2.putText(
        image,
        target,
        (4, max(28, height - 22)),
...
def analyze(video_path: Path, output_path: Path, csv_path: Path, summary_path: Path, config_path: Path) -> None:
    ...
    with csv_path.open('w', newline='') as csv_file:
        fieldnames = [
            ...
            'template_dominance',
            'frame_score',
            'frame_border_density',
            'frame_inner_density',
            'candidate_source',
            'refined_with_white_paper',
            'white_paper_refine_reason',
            'black_frame_quad',
            'white_paper_quad',
            'state',
            'control_mode',
        ...
        while True:
            ...
            _draw_arrow_analysis_overlay(annotated, detection, debug)
            ...
            row = {
                ...
                'frame_score': '0.000000' if detection is None else f'{float(getattr(detection, "frame_score", 0.0)):.6f}',
                'frame_border_density': (
                    '0.000000' if detection is None
                    else f'{float(getattr(detection, "frame_border_density", 0.0)):.6f}'
                ),
                'frame_inner_density': (
                    '0.000000' if detection is None
                    else f'{float(getattr(detection, "frame_inner_density", 0.0)):.6f}'
                ),
                'candidate_source': 'none' if detection is None else str(getattr(detection, 'reject_reason', '')),
                'refined_with_white_paper': (
                    'false' if detection is None
                    else str(bool(getattr(detection, 'refined_with_white_paper', False))).lower()
                ),
                'white_paper_refine_reason': (
                    'none' if detection is None else str(getattr(detection, 'white_paper_refine_reason', ''))
                ),
                'black_frame_quad': 'none' if detection is None else str(getattr(detection, 'black_frame_quad', None)),
                'white_paper_quad': 'none' if detection is None else str(getattr(detection, 'white_paper_quad', None)),
                'state': str(debug.get('state')),
...
def main() -> int:
    ...
    parser.add_argument('--config', type=Path, default=Path('tb4_autonomy_real/config/offline_video_analysis.yaml'))
