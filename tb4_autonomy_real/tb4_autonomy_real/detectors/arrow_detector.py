...
from tb4_autonomy_real.data_types import ArrowDetection, Box2D
...
class ArrowDetectorConfig:
    ...
    use_paper_candidate: bool = True
    black_frame_enable: bool = True
    black_frame_min_area_ratio: float = 0.002
    black_frame_max_area_ratio: float = 0.30
    black_frame_approx_epsilon_ratio: float = 0.04
    black_frame_min_border_density: float = 0.08
    black_frame_max_inner_density: float = 0.35
    black_frame_min_border_inner_contrast: float = 0.03
    black_frame_border_band_ratio: float = 0.12
    black_frame_close_kernel: int = 5
    black_frame_dilate_kernel: int = 3
    black_frame_min_score: float = 0.25
    black_frame_refine_with_white_paper: bool = True
    black_frame_white_roi_padding_ratio: float = 0.18
    black_frame_white_min_area_ratio: float = 0.002
    black_frame_white_max_area_ratio: float = 0.35
    black_frame_white_max_area_expand_ratio: float = 1.8
    black_frame_white_min_overlap_ratio: float = 0.25
    black_frame_white_center_tolerance_ratio: float = 0.35
    black_frame_white_approx_epsilon_ratio: float = 0.035
    black_frame_white_open_kernel: int = 3
    black_frame_white_close_kernel: int = 5
    ...
    use_base_tip_direction: bool = False
...
class _Candidate:
    ...
    frame_border_density: float = 0.0
    frame_inner_density: float = 0.0
...
class ArrowDetector:
    ...
    def __init__(self, config: ArrowDetectorConfig | None = None):
        ...
        self.previous_candidate_box: Box2D | None = None
    ...
    def detect(self, frame, context):
        ...
        # unless a real-scene config explicitly asks to track only black frames.
        used_fallback_candidate = False
        if context is None:
            candidate = self._select_candidate(mask, require_floor=False)
            if candidate is None and self.config.use_paper_candidate:
                candidate = self._select_paper_candidate(scaled_frame, mask)
        else:
            candidate = None
            if self.config.use_paper_candidate:
                candidate = self._select_paper_candidate(scaled_frame, mask)
            if candidate is None:
                if not self.config.use_paper_candidate and self.config.black_frame_enable:
                    candidate = self._select_black_frame_candidate(mask)
                if candidate is None and self.config.merge_black_fragments_fallback:
        ...
        black_frame_quad = None
        white_paper_quad = None
        refined_with_white_paper = False
        white_paper_refine_reason = 'not_attempted'
        if (
            context is not None
            and not self.config.use_paper_candidate
            and self.config.black_frame_enable
            and self.config.black_frame_refine_with_white_paper
            and candidate.reject_reason == 'black_frame'
        ):
            black_frame_quad = np.array(corners_small, dtype=np.float32)
            corners_small, refined_with_white_paper, white_paper_refine_reason = (
                self._refine_black_frame_with_white_paper(
                    scaled_frame,
                    black_frame_quad,
                    mask.shape[:2],
                )
            )
            if refined_with_white_paper:
                white_paper_quad = np.array(corners_small, dtype=np.float32)
        ...
        detection.reject_reason = candidate.reject_reason
        detection.frame_score = candidate.frame_score
        detection.frame_border_density = candidate.frame_border_density
        detection.frame_inner_density = candidate.frame_inner_density
        detection.refined_with_white_paper = refined_with_white_paper
        detection.white_paper_refine_reason = white_paper_refine_reason
        black_frame_quad_full = None if black_frame_quad is None else self._scale_corners(black_frame_quad, scale)
        white_paper_quad_full = None if white_paper_quad is None else self._scale_corners(white_paper_quad, scale)
        detection.black_frame_quad = None if black_frame_quad_full is None else tuple(
            (int(round(float(x))), int(round(float(y)))) for x, y in black_frame_quad_full
        )
        detection.white_paper_quad = None if white_paper_quad_full is None else tuple(
            (int(round(float(x))), int(round(float(y)))) for x, y in white_paper_quad_full
        )
        self.previous_candidate_box = candidate.box

        return detection

    def reset_history(self) -> None:
        self.direction_history.clear()
        self.previous_candidate_box = None
    ...
    def _select_black_frame_candidate(self, mask) -> _Candidate | None:
        height, width = mask.shape[:2]
        image_area = float(width * height)
        if image_area <= 0.0:
            return None
        ...
        floor_mask[:int(height * self.config.floor_roi_min_y_ratio), :] = 0

        close_kernel = self._kernel(self.config.black_frame_close_kernel)
        dilate_kernel = self._kernel(self.config.black_frame_dilate_kernel)
        if close_kernel is not None:
            floor_mask = cv2.morphologyEx(floor_mask, cv2.MORPH_CLOSE, close_kernel)
        if dilate_kernel is not None:
            floor_mask = cv2.dilate(floor_mask, dilate_kernel, iterations=1)
        ...
        for contour in contours:
            ...
            epsilon = max(1.0, self.config.black_frame_approx_epsilon_ratio * perimeter)
            ...
            quad_area_ratio = cv2.contourArea(ordered.astype(np.float32)) / image_area
            if quad_area_ratio < self.config.black_frame_min_area_ratio * 0.5:
                continue

            angle_quality = self._quad_angle_quality(ordered)
            if angle_quality <= 0.10:
                continue

            candidate_quad = ordered
            x, y, w, h = cv2.boundingRect(candidate_quad.astype(np.int32).reshape(-1, 1, 2))
            if w <= 0 or h <= 0:
                continue
            if not self._is_floor_candidate(x, y, w, h, width, height):
                continue
            ...
            if box_area_ratio < self.config.black_frame_min_area_ratio:
                continue
            if box_area_ratio > self.config.black_frame_max_area_ratio:
                continue

            border_margin = max(1, int(min(width, height) * self.config.max_border_touch_ratio))
            touches_left = x <= border_margin
            touches_right = x + w >= width - border_margin
            if y <= border_margin or (touches_left and touches_right):
                continue

            border_density, inner_density = self._black_frame_densities(mask, ordered)
            contrast = border_density - inner_density
            if border_density < self.config.black_frame_min_border_density:
                continue
            if inner_density > self.config.black_frame_max_inner_density:
                continue
            if contrast < self.config.black_frame_min_border_inner_contrast:
                continue

            roi_mask = mask[y:y + h, x:x + w]
            black_ratio = cv2.countNonZero(roi_mask) / box_area

            bottom_score = (y + h) / float(height)
            center_x = x + w / 2.0
            center_error = abs(center_x - width / 2.0)
            center_score = 1.0 - min(1.0, center_error / max(1.0, width * self.config.candidate_max_center_error_ratio))
            area_score = min(1.0, box_area_ratio / 0.06)
            frame_score = self._clamp01(
                0.55 * min(1.0, border_density / 0.45)
                + 0.25 * self._clamp01(contrast / 0.35)
                + 0.20 * angle_quality
            )
            temporal_score = self._candidate_temporal_score(Box2D(x=x, y=y, w=w, h=h), width, height)
            score = (
                0.42 * frame_score
                + 0.20 * area_score
                + 0.17 * bottom_score
            ...
            if score < self.config.black_frame_min_score:
                continue

            candidates.append(_Candidate(
                box=Box2D(x=x, y=y, w=w, h=h),
                contour=candidate_quad.astype(np.int32).reshape(-1, 1, 2),
                area_ratio=box_area_ratio,
                black_pixel_ratio=black_ratio,
                score=score,
                reject_reason='black_frame',
                frame_score=frame_score,
                frame_border_density=border_density,
                frame_inner_density=inner_density,
            ))
    ...
    def _pad_black_frame_quad(self, ordered_quad: np.ndarray, image_width: int, image_height: int) -> np.ndarray:
        x, y, w, h = cv2.boundingRect(ordered_quad.astype(np.int32).reshape(-1, 1, 2))
        pad = int(round(min(w, h) * max(0.0, float(self.config.bbox_padding_ratio))))
        if pad <= 0:
            return ordered_quad

        x1 = max(0, x - pad)
        y1 = max(0, y - pad)
        x2 = min(image_width - 1, x + w + pad)
        y2 = min(image_height - 1, y + h + pad)
        return np.array(
            [[x1, y1], [x2, y1], [x2, y2], [x1, y2]],
            dtype=np.float32,
        )

    def _refine_black_frame_with_white_paper(
        self,
        frame: np.ndarray,
        black_quad: np.ndarray,
        mask_shape: tuple[int, int],
    ) -> tuple[np.ndarray, bool, str]:
        image_height, image_width = mask_shape
        if frame is None or frame.size == 0 or image_width <= 0 or image_height <= 0:
            return black_quad, False, 'invalid_frame'

        ordered_black = self._order_points(np.array(black_quad, dtype=np.float32))
        bx, by, bw, bh = cv2.boundingRect(ordered_black.astype(np.int32).reshape(-1, 1, 2))
        if bw <= 0 or bh <= 0:
            return ordered_black, False, 'invalid_black_bbox'

        padding = int(round(max(bw, bh) * max(0.0, self.config.black_frame_white_roi_padding_ratio)))
        x1 = max(0, bx - padding)
        y1 = max(0, by - padding)
        x2 = min(image_width, bx + bw + padding)
        y2 = min(image_height, by + bh + padding)
        if x2 <= x1 or y2 <= y1:
            return ordered_black, False, 'invalid_roi'

        roi = frame[y1:y2, x1:x2]
        if roi.size == 0:
            return ordered_black, False, 'empty_roi'
        ...
        open_kernel = self._kernel(self.config.black_frame_white_open_kernel)
        close_kernel = self._kernel(self.config.black_frame_white_close_kernel)
        if open_kernel is not None:
        ...
        if not contours:
            return ordered_black, False, 'no_white_contours'

        black_box = Box2D(x=bx, y=by, w=bw, h=bh)
        black_center = np.array(black_box.center, dtype=np.float32)
        black_area = float(max(1, black_box.area))
        ...
        for contour in contours:
            ...
            if local_w <= 0 or local_h <= 0:
                reject_counts['invalid_bbox'] += 1
                continue

            full_box = Box2D(x=x1 + local_x, y=y1 + local_y, w=local_w, h=local_h)
            contour_area = float(cv2.contourArea(contour))
            area_ratio = contour_area / image_area
            if area_ratio < self.config.black_frame_white_min_area_ratio:
                reject_counts['area_too_small'] += 1
                continue
            if area_ratio > self.config.black_frame_white_max_area_ratio:
                reject_counts['area_too_large'] += 1
                continue
            if float(full_box.area) > black_area * self.config.black_frame_white_max_area_expand_ratio:
                reject_counts['expanded_too_much'] += 1
                continue

            refined_center = np.array(full_box.center, dtype=np.float32)
            center_distance = float(np.linalg.norm(refined_center - black_center))
            max_center_distance = max(bw, bh) * self.config.black_frame_white_center_tolerance_ratio
            if center_distance > max_center_distance:
                reject_counts['center_too_far'] += 1
                continue

            overlap_ratio = self._box_intersection_area(full_box, black_box) / black_area
            if overlap_ratio < self.config.black_frame_white_min_overlap_ratio:
                reject_counts['overlap_too_low'] += 1
                continue

            if not self._is_floor_candidate(full_box.x, full_box.y, full_box.w, full_box.h, image_width, image_height):
                reject_counts['not_floor_candidate'] += 1
                continue

            border_margin = max(1, int(min(image_width, image_height) * self.config.max_border_touch_ratio))
            touches_left = full_box.x <= border_margin
            touches_right = full_box.x + full_box.w >= image_width - border_margin
            # The OAK-D preview often sees the current marker clipped by the lower
            # or right image edge. Reject only cases that are clearly not a local
            # floor paper: top-border regions or masks spanning the full width.
            if full_box.y <= border_margin or (touches_left and touches_right):
                reject_counts['border_touch'] += 1
                continue

            perimeter = cv2.arcLength(contour, True)
            epsilon = max(1.0, perimeter * self.config.black_frame_white_approx_epsilon_ratio)
            approx = cv2.approxPolyDP(contour, epsilon, True)
            reason = 'white_approx_quad'
            if len(approx) == 4 and cv2.isContourConvex(approx):
                quad_local = approx.reshape(4, 2).astype(np.float32)
            elif len(contour) >= 5:
                rect = cv2.minAreaRect(contour)
                quad_local = cv2.boxPoints(rect).astype(np.float32)
                reason = 'white_min_area_rect'
            else:
                reject_counts['no_quad'] += 1
                continue

            quad = quad_local + np.array([x1, y1], dtype=np.float32)
            ordered_quad = self._order_points(quad)
            qx, qy, qw, qh = cv2.boundingRect(ordered_quad.astype(np.int32).reshape(-1, 1, 2))
            if qw <= 0 or qh <= 0:
                reject_counts['invalid_quad_bbox'] += 1
                continue
            if not self._is_floor_candidate(qx, qy, qw, qh, image_width, image_height):
                reject_counts['quad_not_floor_candidate'] += 1
                continue

            area_score = min(1.0, area_ratio / max(1e-6, self.config.black_frame_white_min_area_ratio * 6.0))
            overlap_score = min(1.0, overlap_ratio / max(1e-6, self.config.black_frame_white_min_overlap_ratio))
            center_score = 1.0 - min(1.0, center_distance / max(1.0, max_center_distance))
            score = 0.42 * overlap_score + 0.33 * center_score + 0.25 * area_score
            if best is None or score > best[0]:
                best = (score, ordered_quad, reason)

        if best is None:
            if reject_counts:
                reason, _ = reject_counts.most_common(1)[0]
                return ordered_black, False, f'no_valid_white_paper:{reason}'
            return ordered_black, False, 'no_valid_white_paper'
        return best[1], True, best[2]

    def _box_intersection_area(self, first: Box2D, second: Box2D) -> float:
    ...
    def _black_frame_densities(self, mask, ordered_quad: np.ndarray) -> tuple[float, float]:
        size = 120
        dst = np.array(
            [[0, 0], [size - 1, 0], [size - 1, size - 1], [0, size - 1]],
            dtype=np.float32,
        )
        transform = cv2.getPerspectiveTransform(ordered_quad.astype(np.float32), dst)
        warped = cv2.warpPerspective(mask, transform, (size, size))
        _, warped = cv2.threshold(warped, 1, 255, cv2.THRESH_BINARY)

        band = max(3, int(size * self.config.black_frame_border_band_ratio))
        border = np.zeros_like(warped)
        border[:band, :] = 255
        border[-band:, :] = 255
        border[:, :band] = 255
        border[:, -band:] = 255

        inner_margin = min(size // 2 - 1, max(band * 2, int(size * 0.28)))
        inner = warped[inner_margin:size - inner_margin, inner_margin:size - inner_margin]

        border_pixels = max(1, cv2.countNonZero(border))
        border_density = cv2.countNonZero(cv2.bitwise_and(warped, border)) / float(border_pixels)
        inner_density = 0.0 if inner.size == 0 else cv2.countNonZero(inner) / float(inner.size)
        return border_density, inner_density

    def _quad_angle_quality(self, ordered_quad: np.ndarray) -> float:
        angles = []
        points = np.array(ordered_quad, dtype=np.float32)
        for index in range(4):
            ...
            angles.append(angle)
        if min(angles) < 25.0 or max(angles) > 155.0:
            return 0.0
        mean_error = sum(abs(angle - 90.0) for angle in angles) / 4.0
        return self._clamp01(1.0 - mean_error / 75.0)

    def _candidate_temporal_score(self, box: Box2D, image_width: int, image_height: int) -> float:
        if self.previous_candidate_box is None:
            return 0.5
        cx, cy = box.center
        prev_cx, prev_cy = self.previous_candidate_box.center
        distance = math.hypot(cx - prev_cx, cy - prev_cy)
        max_distance = max(1.0, math.hypot(image_width, image_height) * 0.35)
    ...
    def _direction_from_mask(self, contour, mask) -> tuple[str, float]:
        ...
        if self.config.use_base_tip_direction:
            direction, dominance = self._direction_from_base_tip(contour, mask)
    ...
    def _direction_from_base_tip(self, contour, mask) -> tuple[str, float]:
        moments = cv2.moments(contour)
        if abs(moments['m00']) < 1e-6:
            return 'unknown', 0.0
        ...
        heading_angle = self._flip_robot_facing_heading(math.atan2(-dy, dx))
        direction = self._direction_from_angle(heading_angle)
        dominance = max(abs(math.cos(heading_angle)), abs(math.sin(heading_angle)))
        distance_confidence = min(1.0, magnitude / max(1.0, math.hypot(mask.shape[1], mask.shape[0]) * 0.18))
        return direction, max(0.55, min(1.0, 0.65 * dominance + 0.35 * distance_confidence))
    ...
    def _direction_from_vector(self, dx: float, dy: float) -> str:
        return self._direction_from_angle(math.atan2(-dy, dx))

    def _direction_from_angle(self, angle_rad: float) -> str:
