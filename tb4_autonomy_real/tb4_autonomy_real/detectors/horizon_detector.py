...
from tb4_autonomy_real.data_types import HorizonDetection
...
class HorizonDetector:
    ...
    def detect(self, frame, context) -> HorizonDetection | None:
        ...
        # Narrow ROI: focus on the wall/floor boundary band. On the real TB4 the
        # OAK-D cannot be tilted downward like the sim, so keep this configurable.
        top_ratio = float(np.clip(self.roi_top_ratio, 0.0, 0.95))
        bottom_ratio = float(np.clip(self.roi_bottom_ratio, top_ratio + 0.02, 1.0))
        roi_top = int(height * top_ratio)
        roi_bottom = int(height * bottom_ratio)
